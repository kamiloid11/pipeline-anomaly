# src/tests/test_etl_and_anomaly.py
import pandas as pd
...
def test_transform_basic():
    ...
    out = transform(df)
    # ts -> datetime, value -> numeric, forward-fill applied
    assert out['ts'].dtype.kind == 'M'  # datetime
    assert out['value'].dtype.kind in ('f', 'i')  # numeric
    assert len(out) == 3
