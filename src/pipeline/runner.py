...
# optional prometheus metrics module (if exists)
try:
...
def process_file(csv_path: str, archive_dir: Optional[str] = None):
    """
    Single-run processing of one CSV file:
    - extract
    - transform
    - detect anomalies
    - load to ClickHouse
    - return metrics dict
    - move file to archive_dir if provided
    """
    start = time.time()
    ...
    # json metrics for automated consumption
    metrics = {
    ...
    # print JSON to stdout (one-line) so log aggregator can parse it
    ...
    # move processed file to archive_dir
    if archive_dir:
...
def run_once(csv_path: str):
    """Run pipeline once for a path (file or directory)."""
...
def watch_directory(incoming_dir: str, archive_dir: str, poll_interval: int = 5):
    """
    Watch incoming_dir for new CSV files and process them.
    Moves processed files to archive_dir.
    This is a simple polling-based watcher (no inotify).
    """
